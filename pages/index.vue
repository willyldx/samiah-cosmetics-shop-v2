      ...
      <div class="max-w-7xl mx-auto px-4">
        <div class="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 class="text-3xl lg:text-4xl font-bold text-charcoal mt-2">Nos produits</h2>
          </div>
          <NuxtLink to="/produits" class="hidden sm:flex items-center gap-2 text-charcoal font-medium hover:text-gold transition-colors group">
            Voir tout
    ...
    <section class="py-16 lg:py-24">
      <div class="max-w-7xl mx-auto px-4">
        <div class="text-center mb-12">
          ...
          <h2 class="text-3xl lg:text-4xl font-bold text-charcoal mt-2">Ce que disent nos clientes</h2>
        </div>
        ...
        <div v-else-if="testimonials.length" class="grid md:grid-cols-3 gap-6">
          <div v-for="testimonial in testimonials" :key="testimonial.id" class="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow border border-gray-100">
            <div class="flex items-center gap-4 mb-4">
              <div class="w-12 h-12 bg-gold/20 rounded-full flex items-center justify-center">
                <span class="text-gold font-bold">{{ testimonial.client_name ? testimonial.client_name.charAt(0) : '?' }}</span>
              </div>
              <div>
            ...
            <p class="text-gray-600 text-sm line-clamp-3">"{{ testimonial.message }}"</p>
          </div>
        </div>

        <div v-else class="text-center text-gray-500 py-12">Les temoignages arrivent bientot...</div>
      </div>
    </section>

    <section class="py-16 lg:py-24 bg-charcoal text-white">
      <div class="max-w-7xl mx-auto px-4 text-center">
        <h2 class="text-3xl lg:text-4xl font-bold mb-4">Prete a transformer vos cheveux ?</h2>
        <p class="text-white/70 max-w-xl mx-auto mb-8">Reservez votre consultation capillaire personnalisee et decouvrez les soins adaptes a vos besoins.</p>
        <div class="flex flex-wrap justify-center gap-4">
          <a :href="whatsappConsultationLink" target="_blank" rel="noopener" class="inline-flex items-center gap-2 bg-gold text-charcoal px-8 py-4 rounded-full font-bold hover:bg-yellow-400 transition-colors">
          ...
          <NuxtLink to="/produits" class="inline-flex items-center gap-2 bg-white/10 text-white px-8 py-4 rounded-full font-bold hover:bg-white/20 transition-colors">
            Voir les produits
          </NuxtLink>
        </div>
      </div>
...
useHead({
  ...
  meta: [{ name: 'description', content: 'Consultation capillaire en ligne et produits cosmetiques au Tchad.' }]
})
