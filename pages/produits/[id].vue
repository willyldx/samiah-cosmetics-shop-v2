<template>
  <div class="min-h-screen bg-gradient-to-b from-gray-50 to-white">
    ...
    <div v-else-if="!product" class="max-w-7xl mx-auto px-4 py-24 text-center">
      <div class="w-28 h-28 bg-gradient-to-br from-gray-100 to-gray-50 rounded-full flex items-center justify-center mx-auto mb-6 shadow-inner">
        <svg class="w-14 h-14 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
        </svg>
      </div>
      <h1 class="text-2xl font-bold text-charcoal mb-3">Produit introuvable</h1>
      <p class="text-gray-500 mb-8">Ce produit n'existe pas ou a été supprimé.</p>
    ...
    <div v-else class="max-w-7xl mx-auto px-4 py-8 lg:py-12">
      ...
      <nav class="flex items-center gap-2 text-sm text-gray-500 mb-8">
        ...
        <NuxtLink to="/produits" class="hover:text-charcoal transition-colors">Produits</NuxtLink>
        <svg class="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
        <span class="text-charcoal font-medium truncate max-w-[200px]">{{ product.title }}</span>
      </nav>
      ...
      <div class="grid lg:grid-cols-2 gap-8 lg:gap-16">
        ...
        <div class="lg:py-4 space-y-6">
          ...
          <!-- Title -->
          <h1 class="text-3xl lg:text-4xl font-bold text-charcoal leading-tight">
            {{ product.title }}
          </h1>
          ...
          <!-- Description -->
          <p v-if="product.short_description" class="text-gray-600 text-lg leading-relaxed">
            {{ product.short_description }}
          </p>

          <!-- Cities -->
          <div v-if="product.cities && product.cities.length" class="space-y-2">
            <h3 class="text-sm font-bold text-charcoal uppercase tracking-wider">Disponible à :</h3>
            <div class="flex flex-wrap gap-2">
              <span
                v-for="city in product.cities"
                :key="city"
                class="bg-gray-100 text-gray-700 px-4 py-2 rounded-full text-sm font-medium flex items-center gap-1.5"
              >
          ...
          <!-- Quantity -->
          <div class="space-y-3">
            <h3 class="text-sm font-bold text-charcoal uppercase tracking-wider">Quantité :</h3>
            <div class="flex items-center gap-4">
              <button
                type="button"
                @click="decreaseQuantity"
                class="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-all text-lg font-bold disabled:opacity-50"
                :disabled="quantity <= 1"
              >
                -
              </button>
              ...
              <button
                type="button"
                @click="quantity++"
                class="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition-all text-lg font-bold"
              >
                +
              </button>
            </div>
          ...
          <div class="pt-6 border-t border-gray-100">
            <button
              type="button"
              @click="shareProduct"
              class="flex items-center gap-3 text-gray-500 hover:text-charcoal transition-colors group"
            >
